...
train_inp = xt;[1 1; 1 0; 0 1; 0 0];
train_out = rt;[1; 0; 0; 1];

% train_inp = [1; 2; 3; 4; 5; 6; 7;];
% train_out = [1; 4; 9; 16; 25; 36; 49;];
...
for iter = 1:epochs
    ...
    for j = 1:patterns
        ...
        tmp = this_pat*weight_input_hidden;
        for q = 1:length(tmp)
           hval(q) = 1/(1 + exp(-tmp(q)));
        end
        %hval = (tanh(this_pat*weight_input_hidden))';
        sigm = 1 ./ (1 + exp(-this_pat*weight_input_hidden));
        hval = sigm';
        hval = (tanh(this_pat*weight_input_hidden))';%%
        hval = logsig(this_pat*weight_input_hidden)';
    ...
    sigm = 1 ./ (1 + exp(-train_inp*weight_input_hidden));
    pred = weight_hidden_output*logsig(train_inp*weight_input_hidden)';
